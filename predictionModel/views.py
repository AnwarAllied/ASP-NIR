...
class pls(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if model == 'Spectrum':
            data['figure_header'] = 'PLS model for {}:'.format(
                Spectrum.objects.get(id=int(ids.split(',')[0])).origin.split(' ')[0])
        elif model == 'NirProfile':
            data['figure_header'] = 'PLS model for {}:'.format(NirProfile.objects.get(id=int(ids.split(',')[0])).title)
        elif model == 'SgFilter':
            data['figure_header'] = 'PLS model SG filter for {}:'.format(
                SgFilter.objects.get(id=int(ids.split(',')[0])).nirprofile.first().title)
        elif model == 'Ploy':
            data['figure_header'] = 'PLS model for {}:'.format(
                Poly.objects.get(pk=int(ids.split(',')[0])).spectrum.origin.split(' ')[0])
        elif model == 'Match':
...
def pls_save(request):
    if "pls_ids" in request.session.keys():
        ...
        content = {"saved": True, "message": "The model saved successfully, as: " + pls.__str__(),
                   "message_class": "success"}
        # print("x-mean:%s, y-mean:%s,x-std:%s" % (pls.x_mean, pls.y_mean, pls.x_std))
        _ = [request.session.pop(i, None) for i in
             ['pls_ids', 'trans', 'components', 'pls_score', 'pls_mse', 'pls_x_rots', 'pls_x_mean', 'pls_y_mean',
              'pls_coef', 'pls_x_std', 'pls_y_pred', 'preprocessed']]
    else:
...
class PlsScatterChartView(BaseLineChartView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({"title": True if model_id else False, 'text': 'Validation set Rsquared= ' + (
            '00.00' if self.cont['score'] < 0 else "{:0.4f}".format(self.cont['score'])), 'axis_unit': unit})
        return context
    ...
    def spect2context(self, **kwargs):
        ...
        if model == "NirProfile":
            ...
            if mode == 'detail':
                ...
                print('spectra_ids:',[i.id for i in spectra.all()])
            elif model_id:
        ...
        if 'pls' in locals():
            if model_id:  # if avalible: test or show it
                trans, components, score, mse, x_rotations, x_mean, y_mean, coef, x_std, y_pred, ids_filtred, y_true = pls.apply(
                    'test', pls.order, *ids, model=query)
            else:
            ...
            trans, components, score, mse, x_rotations, x_mean, y_mean, coef, x_std, y_pred, ids_filtred, y_true = pls.apply(
                'calibration', components, *ids, model=query)
        ...
        context.update(
            {'model': model, 'Spectra': spectra, 'trans': trans, 'mode': mode, 'y_pred': y_pred, 'score': score,
             'ids_filtred': ids_filtred, 'y_true': y_true})
    ...
    def get_data(self):
        ...
        return [[{"x": a, "y": b}] for a, b in
                zip(y_true, y_pred)]  # [{"x":1,"y":2},{"x":5,"y":4}],[{"x":3,"y":4},{"x":3,"y":1}]]#
...
class pca(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        print('PCA', model, ids)
        if model == 'Spectrum':
            data["figure_header"] = "PCA model for {}:".format(
                Spectrum.objects.get(id=int(ids.split(',')[0])).origin.split(' ')[0])
        elif model == 'NirProfile':
