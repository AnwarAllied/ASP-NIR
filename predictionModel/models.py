...
class PlsModel(models.Model):
    ...
    def __str__(self):
        fname = self.calibration.all()[0].origin.split(' ')[0] + ", score: " + "{:0.2f}".format(
            self.score) + ", mse: " + "{:0.2f}".format(self.mse) + ", comp.: " + str(self.order)
        if self.calibration.count() > 1:
            origin_list = list(set([i.origin.split(' ')[0] for i in self.calibration.all()]))
            if len(origin_list) == 2:
                fname = "%s and %s, score: %s, mse: %s" % (
                origin_list[0], origin_list[1], "{:0.2f}".format(self.score), "{:0.2f}".format(self.mse))
            elif len(origin_list) > 2:
                fname = "%s, %s and %d others, score: %s, mse: %s" % (
                origin_list[0], origin_list[1], self.calibration.count() - 2, "{:0.2f}".format(self.score),
                "{:0.2f}".format(self.mse))
        else:
    ...
    def get_y_train(self, spectra=None):
        # if spectra:
        return np.array([float(findall('\d[\d\.]*', i.origin)[0]) if findall('\d', i.origin) else None for i in
                         (spectra if spectra else self.calibration.all())])
        # else:
    ...
    def __str__(self):
        ...
        if self.calibration.count() > 1:
            ...
            if len(origin_list) == 2:
                ...
                fname = "%s, %s and %d others, score: %s" % (
                origin_list[0], origin_list[1], self.calibration.count() - 2, "{:0.2f}".format(self.score))
